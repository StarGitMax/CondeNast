...
public class ATLAS_610_Sprint_TC_101 extends BaseTest
{
	...
	@Test
	public void VerifyOpportunitywithClosedWonStatus() throws Exception
	{
		sl.NavigateandLogin();
		...
		String sval = opp.fillOpportunitiesForm(AccountName,"5000","30/04/2021","IdentifyingOpportunity","CentrallyBilled");
		String[] sval1 = sval.split(";");
		if(sval1[1].equalsIgnoreCase("10%"))
		{
			reporter.logger.log(Status.PASS, "Probability Value is 10%");
		}
		else
		{
			reporter.logger.log(Status.FAIL, "Probability Value is not 10%");
		}
		...
		opp.CreateQuotes();
		createQuoteLine("Closed Won (100%)");
		sname = getQLIdata();
		...
		{
			reporter.logger.log(Status.PASS, "Probability Value is 100%");
		}
		else
		{
			reporter.logger.log(Status.FAIL, "Probability Value is not 100%");
		}
		opp.SelectBillingType("CentrallyBilled");
	...
	public void createQuoteLine(String stage) throws Exception
	{
		...
		Thread.sleep(10000);
		opp.SelectAvailableMultipleMarkets("GCP HQ;Australia", "2500;2500");
